...
# %%
script = """
masterdir = "/Users/robinyan/Library/Mobile Documents/com~apple~CloudDocs/Downloads/rc1_1day_cdx2_hoxc10_hoxb1_2"
...
//		Determine if stitching and shading correction are needed
		batch_list = getFileList(currentdir);
//		No multiple stage
		if (batch_list.length == 5) {
			print("No need for stitching");
// merge multiple channels into one tiff file and perform standard processing
open(currentdir + File.separator + batch + "_w1405.TIF");
//run("Enhance Contrast", "saturated=0.35");
open(currentdir + File.separator + batch + "_w2488.TIF");
//run("Enhance Contrast", "saturated=0.35");
open(currentdir + File.separator + batch + "_w3561.TIF");
//run("Enhance Contrast", "saturated=0.35");
open(currentdir + File.separator + batch + "_w4640.TIF");
//run("Enhance Contrast", "saturated=0.35");
run("Merge Channels...",
"c1="+
batch + "_w3561.TIF"+" c2="+
...
//			Multiple stage present
		else if (batch_list.length > 5) {
...
// Delete all text and single channel files
list = getFileList(masterdir + File.separator + "Stitched");
for (i=0; i<list.length; i++) {
	if (endsWith(list[i], ".ome.tif")) {
		File.delete(masterdir + File.separator + "Stitched" + File.separator + list[i])
	}
