...
```{r}
genes_to_query <- as.vector(na.omit(as.matrix(genelist[8])))
current <- getwd()
```
...
gao <- rbind(gao_fh1, gao_c5, gao_h1)
...
# Select only day 5
rostovskaya_day5 <- data.matrix(t(rostovskaya[3, ]))
row.names(rostovskaya_day5) <- "Rostovskaya hALEC.Day.5"
...
dong_stb <- dong[c(2, 5), ]
dong_primed <- dong[c(4, 7), ]
dong_naive <- dong[c(6, 1, 3), ]
...
# composite <- rbind(human_gastrula,
# zheng_2, zheng_1, zheng_transwell,
# gao, minn, dong_primed, dong_stb, dong_naive,
# rostovskaya, rostovskaya_late_am,
# rostovskaya_stb,
# liu, yu, io, guo, yanagida,
# blakeley, petropoulos)
composite <- rbind(blakeley, petropoulos,
yanagida, io, dong_naive, guo, yu, rostovskaya_day5,
liu, gao, rostovskaya_late_am, minn, zheng_transwell,
zheng_1, zheng_2, human_gastrula)
composite <- composite[rev(seq_len(nrow(composite))), ] # reverse row name order
composite <- data.matrix(composite)
row.names(composite)[11] <- "Rostovskaya hALEC.Day.5"
# Replace NA with zeros
composite[is.na(composite)] <- 0
# Average human trophoblast
composite[nrow(composite) - 1, ] <-
colMeans(composite[c(nrow(composite) - 1,
nrow(composite)), ])
rownames(composite)[nrow(composite) - 1] <-
"Human Trophoblast"
composite <- composite[-nrow(composite), ]
```
...
saveRDS(composite, file = "composite_list_short.rds")
```
...
```{r}
# Read saved .RDS
composite <- readRDS("composite_list_short.rds")
```
...
```{r}
pdf(file = "heatmap.pdf", width = 4, height = 3)
heatmap
dev.off()
```
...
```{r}
# PCA based on human data
# Split data into human and organoid
composite_human <- composite[13:15, ]
composite_organoid <- composite[- (13:15), ]
# PCA on human data
